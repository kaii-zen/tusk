...
const dockerCompose = (args = [], opts = {}) => exec('docker-compose', [ '--project-name', path.basename(process.cwd()), ...args ], opts)
const startVerdaccio = (net = 'bridge') => dockerCompose([ 'up', '--detach' ], {
  cwd: verdaccioDir,
  env: { net }
})

const stopVerdaccio = () => dockerCompose(['stop'], {
  cwd: verdaccioDir
})

const nukeVerdaccio = () => dockerCompose(['rm', '-vf'], {
  cwd: verdaccioDir
})
